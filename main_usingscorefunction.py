...
if __name__ == "__main__":
    ...
    try :
        ...
        class ScoreFunction(nn.Module):
            def __init__(self, nc= n_ch, nf = n_f, sn= sn):
                super(ScoreFunction, self).__init__()
                self.nc = nc
                if not sn :
                    self.f = nn.Sequential(
                        nn.Conv2d(n_c+1, n_f, 3, 1, 1),
                        nn.LeakyReLU(l),
                        nn.Conv2d(n_f, n_f * 2, 4, 2, 1),
                        nn.LeakyReLU(l),
                        nn.Conv2d(n_f * 2, n_f * 4, 4, 2, 1),
                    ...
                    self.f = nn.Sequential(
                        spectral_norm(nn.Conv2d(n_c+1, n_f, 3, 1, 1)),
                        nn.LeakyReLU(l),
                        spectral_norm(nn.Conv2d(n_f, n_f * 2, 4, 2, 1)),
                        nn.LeakyReLU(l),
                        spectral_norm(nn.Conv2d(n_f * 2, n_f * 4, 4, 2, 1)),
            ...
            def forward(self, x, t,):
                t = t.reshape((t.shape[0], 1, *[1 for _ in range(len(x.shape[2:]))])).expand(t.shape[0], 1, x.shape[2:])
                x = t.cat([x, t], dim=1)
                return self.f(x).squeeze()
