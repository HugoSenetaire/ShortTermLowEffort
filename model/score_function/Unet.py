...
class ScoreFunction(nn.Module):
    ...
    def expand_to_x_k(self, x, k_value):
        assert k_value.shape[0] == x.shape[0]
        k_value = k_value.reshape(x.shape[0], *[1 for _ in range(len(x.shape)-1)]).expand(x.shape[0],1,*x.shape[2:])
        return k_value



    def forward(self, x, k):
        if self.type_method == 'linear':
            x_k = self.expand_to_x_k(x, k/self.K)
            x_k = torch.cat([x, x_k], dim=1)
        elif self.type_method == 'periodic':
            x_sin_k = self.expand_to_x_k(x, math.sin(k/self.K*2*math.pi))
            x_cos_k = self.expand_to_x_k(x, math.cos(k/self.K*2*math.pi))
            x_k = torch.cat([x, x_sin_k, x_cos_k], dim=1)
        return self.model(x_k)
...
class UNetScore(nn.Module):
    def __init__(self, input_nc, output_nc, img_size, ngf, norm_layer_name="instance", use_dropout=False):
        """Construct a Unet generator
        Parameters:
            input_nc (int)  -- the number of channels in input images
            output_nc (int) -- the number of channels in output images
